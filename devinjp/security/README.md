...
1. Create a submodule in [security](/devinjp/security/) with your analyzer's desired name
    * Have your main class inherit from [SecurityAnalyzer](/devinjp/security/analyzer.py)
    * Optional: define API endpoints for `/api/security/{path:path}` to manage settings,
2. Add your analyzer class to the [options](/devinjp/security/options.py) to have it be visible from the frontend combobox
