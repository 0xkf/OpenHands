...
def get_file_handler(
    ...
    file_name = f'devinjp_{timestamp}.log'
...
devinjp_logger = logging.getLogger('devinjp')
current_log_level = logging.INFO

if LOG_LEVEL in logging.getLevelNamesMapping():
    current_log_level = logging.getLevelNamesMapping()[LOG_LEVEL]
devinjp_logger.setLevel(current_log_level)

if DEBUG:
    devinjp_logger.addFilter(StackInfoFilter())

if current_log_level == logging.DEBUG:
    LOG_TO_FILE = True
    devinjp_logger.debug('DEBUG mode enabled.')

if LOG_JSON:
    devinjp_logger.addHandler(json_log_handler(current_log_level))
else:
    devinjp_logger.addHandler(get_console_handler(current_log_level))

devinjp_logger.addFilter(SensitiveDataFilter(devinjp_logger.name))
devinjp_logger.propagate = False
devinjp_logger.debug('Logging initialized')

LOG_DIR = os.path.join(
    # parent dir of devinjp/core (i.e., root of the repo)
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
    'logs',
)

if LOG_TO_FILE:
    devinjp_logger.addHandler(
        get_file_handler(LOG_DIR, current_log_level)
    )  # default log to project root
    devinjp_logger.debug(f'Logging to file in: {LOG_DIR}')
...
class LlmFileHandler(logging.FileHandler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        super().emit(record)
        self.stream.close()
        devinjp_logger.debug('Logging to %s', self.baseFilename)
        self.message_counter += 1
...
class OpenHandsLoggerAdapter(logging.LoggerAdapter):
    ...
    def __init__(self, logger=devinjp_logger, extra=None):
        self.logger = logger
        self.extra = extra or {}
