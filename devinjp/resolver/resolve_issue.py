...
from devinjp.core.config import AgentConfig, AppConfig, LLMConfig, SandboxConfig
from devinjp.core.logger import devinjp_logger as logger
from devinjp.core.main import create_runtime, run_controller
from devinjp.events.action import CmdRunAction, MessageAction
...
from devinjp.events.stream import EventStreamSubscriber
from devinjp.resolver.interfaces.github import GithubIssueHandler, GithubPRHandler
from devinjp.resolver.interfaces.gitlab import GitlabIssueHandler, GitlabPRHandler
from devinjp.resolver.interfaces.issue import Issue
from devinjp.resolver.interfaces.issue_definitions import (
    ServiceContextIssue,
    ServiceContextPR,
)
from devinjp.resolver.resolver_output import ResolverOutput
from devinjp.resolver.utils import (
    Platform,
...
async def resolve_issue(
    ...
    if repo_instruction is None:
        # Check for .devinjp_instructions file in the workspace directory
        devinjp_instructions_path = os.path.join(repo_dir, '.devinjp_instructions')
        if os.path.exists(devinjp_instructions_path):
            with open(devinjp_instructions_path, 'r') as f:  # noqa: ASYNC101
...
def main() -> None:
    ...
    if runtime_container_image is None and not my_args.is_experimental:
        runtime_container_image = (
            f'ghcr.io/all-hands-ai/runtime:{devinjp.__version__}-nikolaik'
        )
