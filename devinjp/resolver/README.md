...
This tool allows you to use open-source AI agents based on [DEVINJP](https://github.com/all-hands-ai/devinjp)
...
3. Copy `examples/devinjp-resolver.yml` to your repository's `.github/workflows/` directory
...
7. Usage:
   ...
   b. Using `@devinjp-agent` mention:
      - Create a new comment containing `@devinjp-agent` in any issue
...
Need help? Feel free to [open an issue](https://github.com/all-hands-ai/devinjp/issues) or email us at [contact@all-hands.dev](mailto:contact@all-hands.dev).
...
```bash
python -m devinjp.resolver.resolve_issue --selected-repo [OWNER]/[REPO] --issue-number [NUMBER]
```

For instance, if you want to resolve issue #100 in this repo, you would run:

```bash
python -m devinjp.resolver.resolve_issue --selected-repo all-hands-ai/devinjp --issue-number 100
```
...
```bash
poetry run python devinjp/resolver/resolve_issue.py --selected-repo all-hands-ai/devinjp --issue-number 100
```

For resolving multiple issues at once (e.g., in a batch process), you can use the `resolve_all_issues` command:

```bash
python -m devinjp.resolver.resolve_all_issues --selected-repo [OWNER]/[REPO] --issue-numbers [NUMBERS]
```

For example:

```bash
python -m devinjp.resolver.resolve_all_issues --selected-repo all-hands-ai/devinjp --issue-numbers 100,101,102
```
...
```bash
python -m devinjp.resolver.send_pull_request --issue-number PR_NUMBER --issue-type pr
```
...
```bash
python -m devinjp.resolver.visualize_resolver_output --issue-number ISSUE_NUMBER --vis-method json
```
...
```bash
python -m devinjp.resolver.send_pull_request --issue-number ISSUE_NUMBER --username YOUR_GITHUB_OR_GITLAB_USERNAME --pr-type draft
```

If you want to upload to a fork, you can do so by specifying the `fork-owner`:

```bash
python -m devinjp.resolver.send_pull_request --issue-number ISSUE_NUMBER --username YOUR_GITHUB_OR_GITLAB_USERNAME --pr-type draft --fork-owner YOUR_GITHUB_OR_GITLAB_USERNAME
```

## Providing Custom Instructions

You can customize how the AI agent approaches issue resolution by adding a `.devinjp_instructions` file to the root of your repository. If present, this file's contents will be injected into the prompt for devinjp edits.
