...
from devinjp.core.config import LLMConfig
from devinjp.core.logger import devinjp_logger as logger
from devinjp.llm.llm import LLM
from devinjp.resolver.interfaces.github import GithubIssueHandler
from devinjp.resolver.interfaces.gitlab import GitlabIssueHandler
from devinjp.resolver.interfaces.issue import Issue
from devinjp.resolver.interfaces.issue_definitions import ServiceContextIssue
from devinjp.resolver.io_utils import (
    load_all_resolver_outputs,
    load_single_resolver_output,
)
from devinjp.resolver.patching import apply_diff, parse_patch
...
def make_commit(repo_dir: str, issue: Issue, issue_type: str) -> None:
    ...
    if not result.stdout.strip():
        ...
        subprocess.run(
            f'git -C {repo_dir} config user.name "devinjp" && '
            f'git -C {repo_dir} config user.email "devinjp@all-hands.dev" && '
            f'git -C {repo_dir} config alias.git "git --no-pager"',
            shell=True,
            check=True,
        )
        logger.info('Git user configured as devinjp')
...
def send_pull_request(
    ...
    base_branch_name = f'devinjp-fix-issue-{issue.number}'
    ...
    if additional_message:
        pr_body += f'\n\n{additional_message}'
    pr_body += '\n\nAutomatic fix generated by [DEVINJP](https://github.com/All-Hands-AI/DEVINJP/) 🙌'
