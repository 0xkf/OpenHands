...
from devinjp import __version__ as oh_version
from devinjp.core.exceptions import AgentRuntimeBuildError
from devinjp.core.logger import RollingLogger
from devinjp.core.logger import devinjp_logger as logger
from devinjp.runtime.builder.base import RuntimeBuilder
from devinjp.utils.term_color import TermColor, colorize
...
class DockerRuntimeBuilder(RuntimeBuilder):
    ...
    def build(
        ...
        if not DockerRuntimeBuilder.check_buildx(self.is_podman):
            # when running devinjp in a container, there might not be a "docker"
            # binary available, in which case we need to download docker binary.
            # since the official devinjp app image is built from debian, we use
            # debian way to install docker binary
            logger.info(
                'No docker binary available inside devinjp-app container, trying to download online...'
            )
            commands = [
                'apt-get update',
