...
from devinjp.core.config import AppConfig, SandboxConfig
from devinjp.core.exceptions import AgentRuntimeDisconnectedError
from devinjp.core.logger import devinjp_logger as logger
from devinjp.events import EventSource, EventStream, EventStreamSubscriber
...
from devinjp.events.serialization.action import ACTION_TYPE_TO_CLASS
from devinjp.integrations.provider import (
    PROVIDER_TOKEN_TYPE,
    ProviderHandler,
    ProviderType,
)
from devinjp.integrations.service_types import Repository
from devinjp.microagent import (
    BaseMicroAgent,
    load_microagents_from_dir,
)
from devinjp.runtime.plugins import (
    JupyterRequirement,
    PluginRequirement,
    VSCodeRequirement,
)
from devinjp.runtime.utils.edit import FileEditRuntimeMixin
from devinjp.utils.async_utils import (
    GENERAL_TIMEOUT,
...
class Runtime(FileEditRuntimeMixin):
    ...
    async def clone_repo(
        ...
        devinjp_workspace_branch = f'devinjp-workspace-{random_str}'
        ...
        checkout_command = (
            ...
            if selected_branch
            else f'git checkout -b {devinjp_workspace_branch}'
        )
    ...
    def maybe_run_setup_script(self):
        """Run .devinjp/setup.sh if it exists in the workspace or repository."""
        setup_script = '.devinjp/setup.sh'
    ...
    def get_microagents_from_selected_repo(
        ...
        microagents_dir = workspace_root / '.devinjp' / 'microagents'
        repo_root = None
        if selected_repository:
            repo_root = workspace_root / selected_repository.split('/')[-1]
            microagents_dir = repo_root / '.devinjp' / 'microagents'
        self.log(
            'info',
        ...
        # Check for legacy .devinjp_instructions file
        obs = self.read(
            FileReadAction(path=str(workspace_root / '.devinjp_instructions'))
        )
        if isinstance(obs, ErrorObservation) and repo_root is not None:
            ...
            self.log(
                'debug',
                f'.devinjp_instructions not present, trying to load from repository {microagents_dir=}',
            )
            obs = self.read(
                FileReadAction(path=str(repo_root / '.devinjp_instructions'))
            )

        if isinstance(obs, FileReadObservation):
            self.log('info', 'devinjp_instructions microagent loaded.')
            loaded_microagents.append(
                BaseMicroAgent.load(
                    path='.devinjp_instructions', file_content=obs.content
                )
            )
