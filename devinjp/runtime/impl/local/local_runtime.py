...
import devinjp
from devinjp.core.config import AppConfig
...
from devinjp.events import EventStream
...
from devinjp.events.serialization import event_to_dict, observation_from_dict
from devinjp.runtime.impl.action_execution.action_execution_client import (
    ActionExecutionClient,
)
from devinjp.runtime.impl.docker.docker_runtime import (
    APP_PORT_RANGE_1,
    APP_PORT_RANGE_2,
    EXECUTION_SERVER_PORT_RANGE,
    VSCODE_PORT_RANGE,
)
from devinjp.runtime.plugins import PluginRequirement
from devinjp.runtime.utils import find_available_tcp_port
from devinjp.runtime.utils.command import get_action_execution_server_startup_command
from devinjp.utils.async_utils import call_sync_from_async
from devinjp.utils.tenacity_stop import stop_if_should_exit


def check_dependencies(code_repo_path: str, poetry_venvs_path: str):
    ERROR_MESSAGE = 'Please follow the instructions in https://github.com/All-Hands-AI/DEVINJP/blob/main/Development.md to install OpenHands.'
...
class LocalRuntime(ActionExecutionClient):
    ...
    def __init__(
        ...
        if self.config.workspace_base is not None:
            ...
            self._temp_workspace = tempfile.mkdtemp(
                prefix=f'devinjp_workspace_{sid}',
            )
            self.config.workspace_mount_path_in_sandbox = self._temp_workspace

        logger.warning(
            'Initializing LocalRuntime. WARNING: NO SANDBOX IS USED. '
            'This is an experimental feature, please report issues to https://github.com/All-Hands-AI/DEVINJP/issues. '
            '`run_as_devinjp` will be ignored since the current user will be used to launch the server. '
    ...
    async def connect(self):
        ...
        code_repo_path = os.path.dirname(os.path.dirname(devinjp.__file__))
