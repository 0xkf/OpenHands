...
from devinjp.runtime.builder import DockerRuntimeBuilder, RuntimeBuilder
...
def prep_build_folder(
    ...
    devinjp_source_dir = Path(devinjp.__file__).parent
    project_root = devinjp_source_dir.parent
    logger.debug(f'Building source distribution using project root: {project_root}')

    # Copy the 'devinjp' directory (Source code)
    shutil.copytree(
        devinjp_source_dir,
        Path(build_folder, 'code', 'devinjp'),
    ...
    for file in ['pyproject.toml', 'poetry.lock']:
        src = Path(devinjp_source_dir, file)
        if not src.exists():
...
def get_hash_for_lock_files(base_image: str) -> str:
    ...
    for file in ['pyproject.toml', 'poetry.lock']:
        src = Path(devinjp_source_dir, file)
        if not src.exists():
            src = Path(devinjp_source_dir.parent, file)
