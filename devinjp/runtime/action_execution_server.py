...
from devinjp_aci.editor.editor import OHEditor
from devinjp_aci.editor.exceptions import ToolError
from devinjp_aci.editor.results import ToolResult
from devinjp_aci.utils.diff import get_diff
from pydantic import BaseModel
from starlette.background import BackgroundTask
from starlette.exceptions import HTTPException as StarletteHTTPException
from uvicorn import run

from devinjp.core.exceptions import BrowserUnavailableException
from devinjp.core.logger import devinjp_logger as logger
from devinjp.events.action import (
    Action,
...
from devinjp.events.event import FileEditSource, FileReadSource
from devinjp.events.observation import (
...
from devinjp.events.serialization import event_from_dict, event_to_dict
from devinjp.runtime.browser import browse
from devinjp.runtime.browser.browser_env import BrowserEnv
from devinjp.runtime.plugins import ALL_PLUGINS, JupyterPlugin, Plugin, VSCodePlugin
from devinjp.runtime.utils.bash import BashSession
from devinjp.runtime.utils.file_viewer import generate_file_viewer_html
from devinjp.runtime.utils.files import insert_lines, read_lines
from devinjp.runtime.utils.memory_monitor import MemoryMonitor
from devinjp.runtime.utils.runtime_init import init_user_and_working_directory
from devinjp.runtime.utils.system_stats import get_system_stats
from devinjp.utils.async_utils import call_sync_from_async, wait_all
...
class ActionExecutor:
    ...
    async def ainit(self):
        ...
        if 'agent_skills' in self.plugins and 'jupyter' in self.plugins:
            obs = await self.run_ipython(
                IPythonRunCellAction(
                    code='from devinjp.runtime.plugins.agent_skills.agentskills import *\n'
                )
            )
    ...
    async def _init_bash_commands(self):
        INIT_COMMANDS = [
            'git config --file ./.git_config user.name "devinjp" && git config --file ./.git_config user.email "devinjp@all-hands.dev" && alias git="git --no-pager" && export GIT_CONFIG=$(pwd)/.git_config'
            if os.environ.get('LOCAL_RUNTIME_MODE') == '1'
            else 'git config --global user.name "devinjp" && git config --global user.email "devinjp@all-hands.dev" && alias git="git --no-pager"'
        ]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--username', type=str, help='User to run as', default='devinjp'
    )
