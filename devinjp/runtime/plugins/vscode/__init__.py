...
from devinjp.runtime.utils.system import check_port_available
...
class VSCodePlugin(Plugin):
    ...
    async def initialize(self, username: str):
        if username not in ['root', 'devinjp']:
            self.vscode_port = None
            self.vscode_connection_token = None
            logger.warning(
                'VSCodePlugin is only supported for root or devinjp user. '
        ...
        cmd = (
            ...
            f'sudo chown -R {username}:{username} /devinjp/.openvscode-server\n'
            'cd /workspace\n'
            f'exec /devinjp/.openvscode-server/bin/openvscode-server --host 0.0.0.0 --connection-token {self.vscode_connection_token} --port {self.vscode_port} --disable-workspace-trust\n'
            'EOF'
        )
